...
# copy-pasted previous class' code; created new objects
...
course_10 <- rbind(largedrop, largeinc) %>%
  select(id, title.new, department.new, diff)

ggplot(data = course_10, mapping = aes(x = title.new, y = diff, fill = department.new)) +
  geom_col() + 
  theme(axis.text.x = element_text(angle = 90))


# employed new table tactics

largerdrop <- largedrop %>%
  select(id, title.new, name.new, department.new, u_grad.new, u_grad.old, diff)

largerdrop %>% 
  gt() %>%
  tab_header(title = 
               "Highest Drops and Gains in Harvard Classes",
             subtitle = "From 9/1/2020 - 9/22/2020") %>%
  cols_label(department.new = "Department", 
             title.new = "Class Title", 
             )

# Some classes may have been dropped due to excessive workload

# "cough cough"

# Some classes may have met at poor times; sections change matters

# some students may have signed up for classes they did not enjoy

# some students could drop from higher classes to lower classes

# some students could get recommendations from new courses
